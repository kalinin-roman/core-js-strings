...
function removeLastOccurrences(str, value) {
  ...
  const start = str.slice(0, indx);
  const end = str.slice(indx + value.length);
  return indx < 0 ? str : start + end;
}
...
function reverseWords(str) {
  const a = str.split(' ');
  return a.map((q) => reverseString(q)).join(' ');
}
...
/**
 * Encode specified string with ROT13 cipherRot13
...
function encodeToRot13(str) {
  ...
  const cipherRot13 = 'nopqrstuvwxyzabcdefghijklmNOPQRSTUVWXYZABCDEFGHIJKLM';
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    if (str[i].match(/[A-Za-z]/)) {
      result += cipherRot13[originalAlpha.indexOf(str[i], 0)];
    } else {
      result += str[i];
    }
  }
